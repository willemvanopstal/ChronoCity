...
/* This is where it all happens! Volumetool
#####################################################
#####################################################
#####################################################
#####################################################
...
function clearInstructions(){
	...
	document.getElementById("runButtonLine").innerHTML = '<button type="button" class="toggleButton" onclick="executeAlgorithm">Run analysis</button>';
}
*/
...
function toggleChanges(){
	var newPoints = document.getElementById("cnew");
	var remPoints = document.getElementById("cremoved");
	var base2 = document.getElementById("baseTwo");
	var base3 = document.getElementById("baseThree");
	//var remPoints = document.getElementById("baseNone");

	checkLayers(base2.checked, base3.checked, newPoints.checked, remPoints.checked);

}

function enable_cd_view(){
	console.log("Boom");
}
...
function updateIndicator(){
	...
	var [AHN2,AHN3,AHN23,AHN32] = identifyBoxes();
	console.log(AHN2.checked, AHN3.checked);
	
	if(AHN2.checked == 1 && AHN3.checked == 0){
		x.innerHTML = "<a><strong>AHN2</strong></a> (2008)";
	}
	else if(AHN2.checked == 0 && AHN3.checked == 1){
		x.innerHTML = "<a><strong>AHN3</strong></a> (2016)";
	}
	else if(AHN2.checked == 1 && AHN3.checked == 1){
		x.innerHTML = "<a><strong>AHN2</strong></a> (2008) & <a><strong>AHN3</strong></a> (2016)";
	}
	else if(AHN2.checked == 0 && AHN3.checked == 0){
		x.innerHTML = "";
	}
}

function getActivePC(){
	var layerList = identifyBoxes();
	var visList = [0,0,0,0];

	for(i=0;i<4;i++){
		visList[i] = layerList[i].checked;
	}
...
function getNumberLoaded(){
	var count = parseInt(document.getElementById("sceneList").childElementCount);
	console.log(count);
	return count;
}

function identifyBoxes(){
	var box1 = document.getElementById("scene_list_item_pointcloud_0");
	var box2 = document.getElementById("scene_list_item_pointcloud_1");
	var box3 = document.getElementById("scene_list_item_pointcloud_2");
	var box4 = document.getElementById("scene_list_item_pointcloud_3");
	var allBoxes = [box1,box2,box3,box4];

	for(i=0;i<4;i++){
	  if(allBoxes[i].nextSibling.data == " AHN2"){
		var AHN2 = allBoxes[i];
	  }
	  else if(allBoxes[i].nextSibling.data == " AHN3"){
		var AHN3 = allBoxes[i];
	  }
	  else if(allBoxes[i].nextSibling.data == " AHNDelta23"){
		var AHNDelta23 = allBoxes[i];
	  }
	  else if(allBoxes[i].nextSibling.data == " AHNDelta32"){
		var AHNDelta32 = allBoxes[i];
	  }
	}

	return [AHN2,AHN3,AHNDelta23,AHNDelta32];
}

function checkLayers(vis2,vis3=99,vis23=99,vis32=99){
	var [AHN2,AHN3,AHN23,AHN32] = identifyBoxes();
	var newPoints = document.getElementById("cnew");
	var remPoints = document.getElementById("cremoved");
	var base2 = document.getElementById("baseTwo");
	...
	if(vis2 == 0 && vis3 == 99 && vis23 == 99 && vis32 == 99) {
        AHN2.checked = 0;
        AHN3.checked = 0;
        AHN23.checked = 0;
        AHN32.checked = 0;
    }
	else if(vis2 == 1 && vis3 == 99 && vis23 == 99 && vis32 == 99){
	 	AHN2.checked = vis2;
		AHN3.checked = 0;
		AHN23.checked = 0;
		AHN32.checked = 0;
		...
		base3.checked = vis3;
		AHN23.checked = vis23;
		newPoints.checked = vis23;
		AHN32.checked = vis32;
		remPoints.checked = vis32;
	}
...
function toggleAHN(){
	...
	if (AHN2.checked == 1 && AHN3.checked == 0){
		checkLayers(0,1,0,0);
		console.log("AHN3 is now visible");
	}
	else if(AHN2.checked == 0 && AHN3.checked == 1){
		checkLayers(1,0,0,0);
		console.log("AHN2 is now visible");
	}
	else {
		console.log("Toggling not available");
	}
}
var gapi_key =  "AIzaSyBHdS_ZCPNH78oHWg4FLT9cD7-7jv7lfoY";
function makeShort(longUrl){
        	// Insert your browser API key generated from Google Developer Console
            gapi.client.setApiKey('AIzaSyBHdS_ZCPNH78oHWg4FLT9cD7-7jv7lfoY');
            ...
            request.execute(function(response)
            {
                console.log("Short URL:"+ response.id);
                                	document.getElementById("sharedLink").value = response.id;
...
xhr.onreadystatechange = function() {
    if (xhr.readyState == 4) {
		...
		if(response != ''){
			...
			var centerstring = obj.response.docs[0].centroide_rd.slice(6,-1).split(" ");
			console.log(obj.response.docs[0].weergavenaam);
			console.log(centerstring);
			console.log("Reloading website");

			viewer.scene.view.position.set(parseFloat(centerstring[0]), parseFloat(centerstring[1]), 300);
			viewer.scene.view.lookAt(new THREE.Vector3(parseFloat(centerstring[0]), parseFloat(centerstring[1]), 0.0));
			viewer.scene.view.yaw = 0.0
		
		}
		else {
			console.log("[ SERVER ERROR! ] Server has not responded, try again.");
			alert("[ SERVER ERROR! ] Server has not responded, try again.");
		}
    }
}
...
var getrequest = "http://geodata.nationaalgeoregister.nl/locatieserver/free?q=" + searchData;
...
xhr.open('GET', "http://localhost:12839/test", true);
xhr.send(null);
console.log("Awaiting response...")
...
function saveAsImage() {
        ...
        try {
            ...
            saveFile(imgData.replace(strMime, strDownloadMime), "ChronoCityExport.png");
...
function shareView(){
	var position = document.getElementById("lblCameraPosition").innerHTML.slice(4).replace(" ","");
	var target = document.getElementById("lblCameraTarget").innerHTML.slice(4).replace(" ","");
	...
	for(i=0;i<4;i++){
		visVar = visVar + String(vis[i] ? 1 : 0) + ";";
	}
	visVar = visVar.slice(0,-1) + "]";
	...
	document.getElementById("sharedLink").value = shareLink;
	return shareLink;

}

function handleChange(responseValue){
	clearRequest();
	document.getElementById("meanWhile").style.display = "none";
	document.getElementById("roiSelecter").style.display = "block";
	...
	//viewer.scene.view.position.set(parseFloat(responseArray[0]), parseFloat(responseArray[1]), parseFloat(responseArray[2])+300);
	//viewer.scene.view.lookAt(new THREE.Vector3(parseFloat(responseArray[0]), parseFloat(responseArray[1]), parseFloat(responseArray[2])));
	//viewer.scene.view.yaw = 0.0
	//http://localhost:8080/chronocity/?position=[85202.24;446842.403333;0]&target=[85202.242;446842.4033331;-3]&vis=[1;0;1;1]
	// Specify default layers to load after completion
...
function disableRun() {
	//document.getElementById("startInfo").innerHTML = "It is first required to specify your region of interest.";
	document.getElementById("runButtonLine").innerHTML = '';
};
...
function clearRequest(){
	...
	document.getElementById('acceptBox').innerHTML = '';
	document.getElementById('insertLine').innerHTML = '<button id="boxbutton" type="button" class="toggleButton" onclick="insertBbox()">Define a new region</button>';
	viewer.scene.removeAllMeasurements();

	// console.log(middle);
	// view.position.set(84426.24,447426.30,58.43);
	// view.lookAt(new THREE.Vector3(middle));
}
...
for(let i = 0; i <= 9; i++){
		currentPoint = document.getElementById("point"+i).innerHTML;
		if(currentPoint != ''){
			numPoints = numPoints + 1;
		};
	};
for(let p = 0; p < (numPoints-1); p++){
	currentPoint = document.getElementById("point"+p).innerHTML.slice(9);
	coord_list = coord_list + "(" + currentPoint + ")";
};
/* alert(coord_list);*/
console.log(coord_list);

clearRequest();
	//document.getElementById("roiSelecter").style.display = "roiSelecter";
	document.getElementById("roiSelecter").style.display = "none";
	//document.getElementById("startAnalysis").style.display = "block";
	document.getElementById("meanWhile").style.display = "block";
	
	document.getElementsByClassName("colorfade")[0].style.color = "red";
setTimeout(function(){document.getElementsByClassName("colorfade")[0].style.color = "black";
}, 4000);
...
console.log("Sending XHR request...");
xhr.open('GET', "http://localhost:12839/?coords="+coord_list, true);
xhr.send(null);
console.log("Awaiting response...")
...
Potree.TransformationTool = class TransformationTool{
	...
	getSelectionBoundingBox(){
		...
		/* Get extents of the bounding box
		#########################################
		#########################################
		#########################################
		#########################################
		...
		var x_min = Math.round(min.x);
		var y_min = Math.round(min.y);
		var z_min = Math.round(min.z);
		var min_extents = x_min + ", " + y_min + ", " + z_min;

		var x_max = Math.round(max.x);
		var y_max = Math.round(max.y);
		var z_max = Math.round(max.z);
		var max_extents = x_max + ", " + y_max + ", " + z_max;

		document.getElementById("extentsHeader").innerHTML = "Extents (x, y, z):";
		document.getElementById("extentsLine_min").innerHTML = "min = " + min_extents;
		document.getElementById("extentsLine_max").innerHTML = "max = " + max_extents;
		console.log(min_extents, max_extents);

		document.getElementById("acceptBox").innerHTML = '<button type="button" class="toggleButton" onclick="clearInstructions()">Accept this region</button>';
...
//------------------------------------------------------------------------------------
	...
	loadSettingsFromURL(){
		...
		if(Potree.utils.getParameterByName("vis")){
			var vis = Potree.utils.getParameterByName("vis");
			console.log(vis);
			var visList = vis.replace("[","").replace("]","").split(";");
			console.log(visList);
			for(var i=0; i<visList.length;i++) visList[i] = parseInt(visList[i], 10);
			console.log(visList);
			checkLayers(visList[0],visList[1],visList[2],visList[3]);
		}
...
//------------------------------------------------------------------------------------
	...
	loadGUI(callback){
		...
		sidebarContainer.load(new URL(Potree.scriptPath + "/sidebar.html").href, () => {
			...
			sidebarContainer.css("width", "350px");
...
function initMeasurementDetails(){
	...
	var trackMeasurement = function(scene, measurement){
		...
		if(elPanelStretch.innerHTML != "Area") {
		...
		var updateDisplay = function(event){
			...
			for(var i = 0; i < positions.length; i++){
				...
				var pointLine = "point" + i;
				if(measurement instanceof Potree.Measure && measurement.showArea){
						console.log(i, txt);
						document.getElementById(pointLine).innerHTML = "point" + i + ": &emsp;" + txt;
