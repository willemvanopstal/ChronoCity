...
def showWKT():
    try:
        if 'coords' in request.args:
            ...
            (ax, ay, az, new_file) = handle_coords(coords, sys_par)  # Here, the above function is called using the variable
            middle = ax + ', ' + ay + ', ' + az
            # The return values are stored in another one.

            if new_file is not False:
                # Write a new Shapefile to be used as the new single clip polygon
                with fiona.open(new_file[1], 'w', 'ESRI Shapefile',
                                {'geometry': 'Polygon', 'properties': {'id': 'int'}}) as c:
                    c.write({'geometry': mapping(new_file[0]), 'properties': {'id': new_file[2]}})
